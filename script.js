const spaceId = 'af6w10ss53ln'; 
const accessToken = 'qc2H3LyqtPNIUWJJ0Yi2IxoHJfqygoxt2TElUP1hKZo';
...
// --- DİNAMİK CONTENTFUL İLAN ÇEKME FONKSİYONU ---
async function loadPropertiesFromContentful() {
    if(!spaceId || spaceId === 'BURAYA_SPACE_ID_YAZ') return;
    
    const apiUrl = `https://cdn.contentful.com/spaces/${spaceId}/entries?access_token=${accessToken}&content_type=property`;
    ...
    try {
        ...
        data.items.forEach((item, index) => {
            ...
            let imageUrl = 'assets/istanbul-bosphorus-fallback.jpg'; // Yedek fotoğraf
            
            // FOTOĞRAF HATASINI ÇÖZEN KISIM (Many files veya One file fark etmeksizin çalışır)
            let targetAssetId = null;
            if (fields.image) {
                if (Array.isArray(fields.image) && fields.image.length > 0) {
                    targetAssetId = fields.image[0].sys.id; // Many files seçildiyse ilkini al
                } else if (fields.image.sys) {
                    targetAssetId = fields.image.sys.id; // One file seçildiyse
                }
            }

            if (targetAssetId && data.includes && data.includes.Asset) {
                const asset = data.includes.Asset.find(a => a.sys.id === targetAssetId);
                if (asset) imageUrl = 'https:' + asset.fields.file.url;
            }

            // Animasyon gecikmesi (kartlar sırayla gelsin diye)
            let delayClass = "";
            if (index % 3 === 1) delayClass = "delay-1";
            if (index % 3 === 2) delayClass = "delay-2";
            
            // Satır aralarını ve tırnakları düzeltiyoruz (hata vermemesi için)
            const safeTitle = fields.title ? fields.title.replace(/'/g, "\\'") : '';
            const safeLoc = fields.location ? fields.location.replace(/'/g, "\\'") : '';
            const safePrice = fields.price ? fields.price.replace(/'/g, "\\'") : '';
            const safeType = fields.type ? fields.type.replace(/'/g, "\\'") : '';
            const safeSize = fields.size ? fields.size.replace(/'/g, "\\'") : '';
            const safeDesc = fields.description ? fields.description.replace(/\n/g, ' ').replace(/'/g, "\\'") : '';

            const cardHtml = `
                <div class="property-card reveal active ${delayClass}" onclick="openPropertyModal('${safeTitle}', '${safeLoc}', '${safePrice}', '${safeType}', '${safeSize}', '${safeDesc}', '${imageUrl}')">
                    <div class="card-img">
                    ...
                    <div class="card-body">
                        ...
                        <div class="card-footer"><span data-i18n="view_dossier">View Dossier</span><span>→</span></div>
                    </div>
                </div>
            `;
        ...
        // İlanlar yüklendikten sonra çevirileri yeni gelen kartlara da uygula
        applyLanguage(currentLang);

    } catch (error) {
        console.error("Contentful API Hatası:", error);
        document.getElementById('portfolio-grid').innerHTML = '<p style="text-align:center; color:white;">İlanlar yüklenirken bir hata oluştu.</p>';
    }
}
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    loadPropertiesFromContentful(); // Verileri çekmeye başla
